...
describe("AWS Blocks 永続バックエンド経由のフロー（#15, ADR-AB11）", () => {
  ...
  it("リロード相当（同一 scope で再オープン）しても予約データ（占有・スペース）が保持される", async () => {
    ...
    // 注: 予約番号＋メール照会は顧客プロフィール（連絡先）に依存するが、プロフィールは
    // 現状インメモリ共存（認証=Cognito へ移行済・プロフィール DB Block 化は後続, ADR-AB07 / §9）。
    // よって本テストでは Database 永続化済みの「占有・スペース」で保持を検証する。
  });
});
