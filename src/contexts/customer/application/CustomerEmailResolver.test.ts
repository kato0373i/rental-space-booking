...
describe("CustomerEmailResolver（通知の宛先解決, #11）", () => {
  it("登録済み顧客の実メール（小文字化済み）を返す", () => {
    ...
    expect(resolver.realEmailFor(guest.id)).toBe("guest@example.com");
  });

  it("未登録 CustomerId は undefined（送信スキップの判断に使う）", () => {
    const resolver = new CustomerEmailResolver(new InMemoryCustomerRepository());
    expect(resolver.realEmailFor(CustomerId.of("missing"))).toBeUndefined();
  });
});
