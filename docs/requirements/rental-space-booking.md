...
| 作成日 | 2026-06-23 |
| 最終更新 | 2026-06-23 |
| 関連ドキュメント | 設計書: `docs/design/rental-space-booking.md`（未作成） |
...
Scenario: 料金表に該当しない時間帯
  ...
  When 単価を参照する
  Then 設定不備として検出され、そのスロットは予約不可（または既定単価）として扱われる
```
...
- **概要**: ゲストが連続スロット群を指定して予約を作成する。フローは「予約をPendingで作成 → スロット占有 → 決済実行 → 成功でConfirmed／失敗でPending破棄・スロット解放」。
- **関連ストーリー**: US-003
- **優先度**: Must
...
- **概要**: 予約の確定（Pending作成時のスロット占有を含む）において、対象スロットが既に占有されている場合は後勝ちを拒否する。仮押さえUIは持たず、確定時点の在庫整合性で担保する。
- **関連ストーリー**: US-003
- **優先度**: Must
...
Scenario: 同一スロットへの競合予約は一方のみ成功
  ...
  Then 先に在庫を確保した1件のみが確定し、もう1件は「すでに予約されました」エラーになる

Scenario: 占有確保とコミットの間に横取りされた場合
  Given ゲストAの予約確定処理中に、ゲストBが同一スロットを確定した
  When ゲストAの確定がコミットされる
  Then 重複が検出され、ゲストAの予約は確定されず（決済も巻き戻り）エラーになる
```
...
Scenario: キャンセル不可期間／終端状態
  Given 予約が既に「完了」または「キャンセル済」である
  When キャンセルしようとする
  Then 「この予約はキャンセルできません」エラーになる（終端状態は不変）
```
...
| NFR-002 | セキュリティ | 認証はモック。実カード情報は扱わない（決済モック）。個人情報（氏名/メール/電話）はインメモリ保持のみとし、ログへ平文出力しない | 決済情報がドメイン/ログに保存されないこと、認可（管理者/ゲスト）が分離されていること |
...
stateDiagram-v2
    ...
    Pending --> [*]: 決済失敗／タイムアウト（破棄・スロット解放）
    Confirmed --> Cancelled: キャンセル（締切＋料率）／管理者強制
    Confirmed --> Completed: 利用終了時刻の経過
...
- **終端状態**: `Cancelled` / `Completed` / `NoShow`。終端状態の予約は編集・キャンセル不可。
- **スロット占有**: `Pending` と `Confirmed` がスロットを占有する。`Cancelled` への遷移と `Pending` の破棄でスロットを解放する。
- **`Pending` の破棄**は永続的な状態遷移先を持たず、予約自体を成立させない（在庫を解放して消滅）。トレーサビリティ目的で「不成立(Aborted)」終端状態を別途設けるかは設計で判断する（未解決事項 U-04）。
...
| # | 論点 | 担当 | 期限 |
|---|---|---|---|
| U-01 | キャンセルポリシー（締切時間・料率の具体値）はスペース共通か、スペースごとに設定可能か | ユーザー | 設計前 |
| U-02 | 料金表の区分粒度（曜日×時間帯の境界。祝日の扱い、日跨ぎ営業の有無） | ユーザー | 設計前 |
| U-03 | リマインド送信タイミング（24h前固定で良いか、複数回送るか） | ユーザー | 設計前 |
| U-04 | 決済失敗による不成立予約を「Aborted」終端状態として残すか、完全に消すか（トレーサビリティ要否） | 設計 | 設計時 |
| U-05 | 会員と予約の関連（会員IDで予約を束ねるか、ゲスト予約を後から会員に紐付けるか） | ユーザー | 設計前 |
| U-06 | 強制キャンセル時のキャンセル料0%上書きを誰がどの条件で許可するか | ユーザー | 設計前 |
...
| 予約 (Reservation) | あるゲストが、1スペース内の連続スロット群を確保する集約。状態（Pending/Confirmed/Completed/Cancelled/NoShow）を持つ。 |
